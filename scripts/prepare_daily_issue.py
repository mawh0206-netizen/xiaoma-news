...
def detail_body(story: dict, deep: bool = False) -> str:
    ...
    supplements = [
        f"信息边界：当前材料能够说明事件方向，但不足以替代完整公告或原始数据。涉及{category}的具体规模、适用对象和生效条件，应以相关主体后续正式文件为准。",
        "比较视角：评估这一变化时，还应与同类公司、历史周期和行业平均水平比较。只有当多项独立指标出现一致变化，才更有理由判断它已经成为趋势。",
        "执行观察：短期关注政策或企业表态，中期关注产品、订单和客户采用，长期则要回到盈利质量与现金流。三个时间尺度的结论可能不同，不宜相互替代。",
        "风险提示：公开报道可能随着新信息出现而修订，市场价格也可能提前反映预期。阅读时应保留来源、发布时间和待确认事项，避免忽略信息时差。",
    ]
